...
async function listBucket (s3Client, bucket, songListOnly) {
  ...
  do {
    ...
    if (songListOnly)
      newContents = newContents.filter((item) => {
...
export default async function s3Download (
  ...
  try {
    ...
    const newContents = s3Contents.filter(
      (item) => !localContents.includes(item.LocalPath)
    )
    ...
    while (localContents.length) {
      ...
      const contentIndex = s3Contents.findIndex(
        (item) => item.LocalPath === localFile
      )
